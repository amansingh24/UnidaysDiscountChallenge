...
public class UnidaysDiscountChallenge {
	...
	public int CalculateTotalPrice (ArrayList <Item> myList) { //this calculates the total price of the items in the basket
		...
		product1.setPriceRule(8); //sets the price rules
		product1.setName('A'); //sets the item type
		Item product2 = new Item();
		product2.setPriceRule(12);
		product2.setName('B');
		Item product3 = new Item();
		product3.setPriceRule(4);
		product3.setName('C');
		Item product4 = new Item();
		product4.setPriceRule(7);
		product4.setName('D');
		Item product5 = new Item();
		product5.setPriceRule(5);
		product5.setName('E');
		
		ArrayList <Item> productCat = new ArrayList<Item>();
		
		UnidaysDiscountChallenge example = new UnidaysDiscountChallenge();
		
		example.AddToBasket(product1,productCat);
		example.AddToBasket(product2,productCat); //adds both products to the basket
		example.AddToBasket(product3,productCat);
		example.AddToBasket(product4,productCat);
		example.AddToBasket(product5,productCat);
		...
		while (z < productCount.size()) { //loops through the basket again
			Item temp3 = productCat.get(z); //stores the item
			...
			char letter = productList.get(z); //gets the name of the item type
